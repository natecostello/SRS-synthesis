...
def run_tom_irvine_ws_case():
    """
    Run Tom Irvine's wavelet synthesis case and save results to file.
    
    This demonstrates wavelet synthesis with the specified SRS requirements:
    - SRS spec: [10 9.4; 80 75; 2000 75] 
    - Octave spacing: 1/12
    ...
    print("Tom Irvine's Wavelet Synthesis Case")
    print("=" * 40)
    
    # SRS specification: srs_spec=[10 9.4; 80 75; 2000 75]
    freq_spec = np.array([10.0, 80.0, 2000.0])
    accel_spec = np.array([9.4, 75.0, 75.0])
    ...
    ntrials = 75                       # reduced for quick testing
    
    # All optimization weights = 1.0 (default values)
    weights = {
        'iw': 1.0,      # SRS error weight
        'ew': 1.0,      # Total error weight  
        'dw': 1.0,      # Displacement weight
        'vw': 1.0,      # Velocity weight
        'aw': 1.0,      # Acceleration weight
        'cw': 1.0,      # Crest factor weight
        'kw': 1.0,      # Kurtosis weight
        'dskw': 1.0     # Displacement skewness weight
    }
    ...
    output_file = 'tom_irvine_ws_case_results.pkl'
